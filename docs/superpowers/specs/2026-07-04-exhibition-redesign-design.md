...
- **M1 · REVISED (Sanya, 2026-07-04): the hero-to-About bead thread and scroll stub were built, reviewed, and then REMOVED at Sanya's request** (the floating orchid lines read as stray scribbles crossing the vignette cards). The thread spine now begins at the untangling map's tangle→straight connectors. Do not reintroduce floating connector lines between the hero and About.

**Post-build amendments (Sanya, 2026-07-04):** origin vignettes carry V2-style microcopy — an origin intro line ("computer-science engineer… it found me"), numbered kickers (`01 — The workshop`), richer bodies (Broadcom workshop / dad+GPay quiet ease / Jobs-bio product-success), and handwritten Caveat taglines ("People first. Always." · "The best UX is invisible." · "Design is a philosophy."). **Accessibility model change:** text tokens (`--ink`/`--muted`) are NOT linearly interpolated with the atmosphere — they derive from the current background lightness (dark text over light worlds, light over dark, smoothstep hand-off at bg L 0.48–0.62), guaranteeing readable contrast at every temperature; glass is de-hazed (single edge highlight, blur 12px, wash alpha ≤0.22, grain 0.03).
